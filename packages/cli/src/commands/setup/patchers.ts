export function patchNextConfigImpl(src: string): string | null {
  ...
  if (hasExport) {
    ...
    const result = patterns.reduce(
      (acc, pattern) => acc.replace(pattern, (_match, expr) => `export default withTailwindStyled()(${expr})`),
      withImport
    )
...
export function patchTsConfigImpl(src: string): string | null {
  try {
    ...
    const updatedCompilerOptions = {
      ...
      moduleResolution: 
        compilerOptions.moduleResolution === "node16" || compilerOptions.moduleResolution === "bundler"
